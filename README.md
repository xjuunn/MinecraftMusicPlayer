...
- 支持歌单播放模式，可从歌单第一首自动顺序播放到最后一首
- 当前播放提供可点击的歌曲直链按钮，可在浏览器中打开下载
- 歌曲开始播放时，聊天栏的 `正在播放` 广播消息也会附带可点击的作者详情和 `下载` 按钮
- 搜索结果分页，并带可点击的 `上一页` / `下一页` 按钮
- 队列列表支持分页，并且歌曲名、作者名、歌单创建者、用户信息都可点击查看详情
- 在待播列表中点击 `下一首` 按钮，可以把对应歌曲提升到队列顶部并作为下一首播放
- 搜索页、详情页和队列页提供统一的快捷入口，可快速跳转到当前播放、播放队列和关联详情
- 歌单详情、用户歌单、作者详情会按批次完整拉取数据，不再只显示前几条
- 自动播放下一首
- 服务端会自动预缓存队列前几首歌曲，默认预缓存 `3` 首，减少切歌时的等待
- 玩家投票切歌
- 重复点歌智能去重，快速连续点击不会把同一首歌反复塞入队列
...
- 聊天栏界面采用统一的标题、快捷入口、列表项和分页条结构，信息层级更清晰
...
| --- | --- |
| `/music` | 显示帮助 |
| `/music now` | 查看当前播放，并提供可点击的歌曲直链 |
| `/music queue` | 查看当前队列 |
| `/music queue next <歌曲ID>` | 将待播列表中的某首歌提升为下一首播放 |
| `/music join` | 加入当前播放 |
| `/music leave` | 退出当前播放 |
| `/music mute once` | 仅停止接收当前歌曲 |
| `/music vote next` | 投票切到下一首 |
| `/music play song <歌曲ID>` | 直接点播单曲 |
| `/music play playlist <歌单ID>` | 切换到歌单播放模式，并从第一首开始顺序播放 |
...
当接口没有直接提供总量时，模组会按页持续拉取数据，并依据“当前批次返回数量是否小于页面大小”来判断是否还有下一页。
这条规则同时用于搜索结果和需要增量拉取的详情列表。

在歌单详情页中，顶部按钮为 `播放歌单`：

- 点击后会清空当前待播队列
- 立即切换到该歌单的播放模式
- 从歌单第一首开始，按顺序自动播放到最后一首
- 如果受配置上限限制，会提示有多少首未被载入
...
| `/music admin set queueCacheSize <0-20>` | 设置服务端预缓存的队列歌曲数量，`0` 为关闭 |
...
- 服务端会自动请求并缓存待播队列前几首歌曲的解析结果
- 默认缓存数量为 `3`
- 当切换到下一首歌时，会优先复用缓存结果，减少等待时间
- 管理员可以通过 `/music admin set queueCacheSize <0-20>` 调整缓存数量
- 设置为 `0` 时禁用队列预缓存
